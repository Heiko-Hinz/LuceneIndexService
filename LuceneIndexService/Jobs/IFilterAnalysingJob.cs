...
{
    public class IFilterAnalysingJob : BaseAnalysingJob
    {
        ...
        public IFilterAnalysingJob() : base()
        {
            Init();
        }
        public IFilterAnalysingJob(ServiceIndex index, Web web, Settings.FileType fileSettings, string path, Uri url, List<string> folderAuthorizedRoles) : 
        ...
        private void Init()
        {
            ...
            Description = "Analysiert ein Dokument mit Hilfe des im System zugeordneten iFilters und fügt es dem Index hinzu.";
            ...
            try
            {
                ...
                foreach (DataProperty property in FileSettings.Properties.Where(p => p.Source == DataSources.DocumentContent))
                {
                    if (property.Name == "content")
                    {
                        try
                        {
                            FilterReader reader = new FilterReader(Path);
                            if (reader.Text != null)
                            {
                                object value = reader.Text;

                                value = property.PerformTasks(value);

                                AbstractField field = property.GetDocumentField(value.ToString().Trim());
                                document.Add(field);
